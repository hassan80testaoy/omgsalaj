...
$(document).ready(function() {
  ...
  function insertAfter(addition, target) {
    ...
    if (parent.lastChild() == target) {
      ...
      parent.insertBefore(addition, target.next());
    }
  }

  var adscont = $("#adsCent");
  var target = $("#adsCenter");

  var bungkus3 = target.find("br");
  if (bungkus3.length > 0) {
    insertAfter(adscont, bungkus3.eq(2));
  }

  var bungkus = target.find("blockquote");
  if (bungkus.length > 0) {
    insertAfter(adscont, bungkus.eq(0));
  }

  var bungkus2 = target.find("h2");
  if (bungkus2.length > 0) {
    insertAfter(adscont, bungkus2.eq(0));
  }
});
